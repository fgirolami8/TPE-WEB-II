...
class MusicController {
    ...
    //ver home
    function viewHome(){
        $this->view->showHome($this->authHelper->log_state(), $this->authHelper->rol_state());
    }
    ...
    function viewSongs(){
        ...
        $songs = $this->model_songs->getSongs();
        $this->view->showSongs($songs, $artistasFromDB, $this->authHelper->log_state(), $this->authHelper->rol_state());
        
    }
    function viewSong($id){
        $song = $this->model_songs->getSong($id);
        $this->view->showSong($song, $this->authHelper->log_state(), $this->authHelper->rol_state());
    }

    //ver artistas
    function viewArtists(){
        $artists = $this->model_categories->getArtists();
        $this->view->showArtists($artists, $this->authHelper->log_state(), $this->authHelper->rol_state());
    }
    function viewArtist($id){
        $artist = $this->model_categories->getArtist($id);
        $this->view->showArtist($artist, $this->authHelper->log_state(), $this->authHelper->rol_state());
    ...
    function addSong(){
        ...
        if ($rol == "true") { //admin == true / user == false
    ...
    function editSong($song){
        ...
        $rol = $this->authHelper->rol_state();
        if ($rol == "true") { //admin == true / user == false
            $this->comprobar_existencia_cancion(); // HAY UN PROBELMA CON ESTA FUNCION
